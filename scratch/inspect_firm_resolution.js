...
async function inspectFirms() {
  ...
  (sjcData || []).forEach((row) => {
    ...
    const firm = sfFirmMap.get(`${sfNo}::${pName}`) || sfFirmMap.get(sfNo) || "";
    if (sjcNo && firm) {
      sjcFirmMap.set(`${sjcNo}::${pName}`, firm);
      sjcFirmMap.set(sjcNo, firm);
    }
  });

  console.log("=== All semi_actual rows with Processing Cost > 0 ===");
  (semiActualData || []).forEach(row => {
    ...
    if (procCost > 0) {
      console.log(`ID: ${row.id}, DirectFirm: "${directFirm}", ResolvedFirm: "${resolvedFirm}", SJC: "${sjcNo}", SF: "${sfNo}", Product: "${prodName}", Cost: ${procCost}`);
    }
  });
}
